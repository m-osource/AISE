...
* **Budget-Optimized Dual Sandbox vs. 3-Machine Secure Ecosystem:** While this PoC implements a dual-hardware layout using Linux Network Namespaces to balance budget constraints, the architecture natively scales into a **three-machine physical deployment** to neutralize rogue or autonomous AI models. By decoupling the architecture into three hardware layers, the system locks the AI inside an isolated compute-only zone, stripped of WAN interfaces, ensuring that a kernel panic or Ring 0 breakout on the GPU node cannot bypass the perimeter.
