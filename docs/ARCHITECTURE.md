...
- [Initial Packets Inspection](#1-initial-packet-inspection)
- [Tcp Handshake Handling](#2-tcp-handshake-handling)
- [Session Greeting](#3-session-greeting)
- [The OpenBSD Architecture](#4-openbsd-architecture)
- [The Fash Path Promotion](#5-fast-path)
- [The List of the eBPF maps](#6-map-list)
- [Penalities Management](#7-penalities-management)
- [Closing Session Stage](#8-connection-closing)
- [Appendix A: The Linux Optimizations](#appendix-a-linux-environments)
- [Architecture Resilience](#9-final-considerations-on-architectural-resilience)
- [Standards References and Bibliography](#standards-references-and-bibliography)
- [Licensing & Commercial Terms](#licensing-and-commercial-terms)
...
<a id="2-tcp-handshake-handling"></a>
...
<a id="7-penalities-management"></a>
...
<a id="appendix-a-linux-environments"></a>
...
1. **NUMA Topological Separation (Cache Thrashing Prevention):**
   * **NUMA Node 0 (Network Fast-Path):** Dedicated exclusively to the Network Interface Card (PCIe NIC), hardware RSS queues, and XDP/eBPF program execution. Keeps session tables warm inside L1/L2/L3 caches of CPU cores reserved strictly for network I/O.
   ...
   * **NUMA-Aware Map Allocation:** To prevent cross-socket latency, all pinned eBPF maps must be explicitly instantiated within NUMA Node 0 memory banks. The AI Inferer Engine on Node 1 interacts with these maps via standard cross-socket links, preserving the sub-microsecond determinism of the Node 0 network fast-path.
   * **Programmatic NUMA Enforcement:** To guarantee that pinned eBPF maps are physically instantiated within NUMA Node 0 memory banks, the C++ user-space orchestrator leverages native kernel APIs. It enforces strict memory and thread affinity by utilizing the `BPF_F_NUMA_NODE` flag via `libbpf` during map initialization, combined with `libnuma` (`numa_sched_setaffinity`) to bind the loader daemon to Node 0 CPU cores. This eliminates external runtime orchestration dependencies (such as `numactl`) and guarantees that high-frequency network interactions completely bypass cross-socket Inter-Connect traversal.

2. **In-Kernel Packet Steering & Hardware IRQ Alignment (`SO_ATTACH_REUSEPORT_EBPF`):**
   * **In-Kernel Connection Steering:** Once incoming traffic from OpenBSD is processed through the standard Linux network stack on the dedicated fiber interface, socket selection utilizes an eBPF program attached via `SO_ATTACH_REUSEPORT_EBPF` (using the `BPF_PROG_TYPE_SK_REUSEPORT` infrastructure). This allows the kernel to steer incoming `SYN` packets directly to the localized worker thread of the **Gatekeeper**, bypassing traditional user-space connection locks.
   * **Cross-NUMA Map Interoperability:** In the event that user-space programs residing on a different NUMA node need to interact with kernel-level states, the system leverages pinned eBPF maps to bridge the topological boundary. To achieve sub-microsecond retrieval efficiency, the in-kernel steering program calls the `bpf_get_numa_node_id()` helper to dynamically select local socket lookup tables (`BPF_MAP_TYPE_REUSEPORT_SOCKARRAY`) relative to the executing CPU core, while cross-node user-space threads safely query the shared pinned maps over the standard interconnect.
   * **Hardware IRQ Affinity Optimization:** To maximize throughput on the non-XDP fiber interface, hardware Interrupt Requests (IRQs) are explicitly bound to the specific NUMA node managing the target application layer. By configuring `/proc/irq/{IRQ}/smp_affinity_list`, the hardware network descriptor ring processing matches the exact CPU cores where the **Gatekeeper** threads reside. This prevents expensive context switches and inter-socket Inter-Connect thrashing during heavy packet ingestion.
   * **AI Compute Execution Constraints:** To mirror the network isolation plane, the heavy compute workload handled by the **AI Inferer Engine** must be strictly confined to **NUMA Node 1** hardware blocks. When initializing the inference engine (e.g., executing the `mistralrs` binary), the runtime environment enforces total isolation from the network processing cores via `numactl`:
     ```bash
     numactl --membind=1 --cpunodebind=1 ./mistralrs --port 8080 --model-id /path/to/model
     ```
     This execution constraint guarantees that the high-density matrix multiplications and large language model (LLM) weights reside entirely within Node 1 RAM banks, ensuring that compute spikes never induce hardware memory bus contention or cache pollution on the Node 0 network gateway.
...
<a id="9-final-considerations-on-architectural-resilience"></a>
...
* **Decoupled and Stateless Authentication (Blind Verification):** 
  The Auth Verifier Daemon manages user-space authentication independently in a fully stateless manner. If the `AUTH_CK` UDP probe fails to receive the `AUTH_UN` hardware confirmation from XDP (i.e., non-existent or purged tuple in `map_session_v4 / v6`), the daemon triggers an immediate failure without hitting the database or consuming CPU cycles. No process restarts or memory rotations are required for single authentication failures, as stream cleanup occurs directly at the hardware/eBPF layer:
...
<a id="standards-references-and-bibliography"></a>
...
<a id="licensing-and-commercial-terms"></a>
