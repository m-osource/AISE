#######################################
# :rocket: Key Architectural Innovations
...
* **Blind Authentication & Synchronous 2-Way Control-Loop (`AUTH_CK` / `AUTH_UN` / `AUTH_OK`):** The user-space Auth Verifier remains completely "deaf" to incoming credentials until XDP confirms the L4 flow state via in-kernel `XDP_TX` probes. This fundamentally eliminates enumeration attacks, brute-force vectors, and internal database exposure to unvalidated sockets.
...
* **Level 0 â€” Master Root Daemon (Privileged Key Custodian):**
  * Holds `root` privileges, manages the primary process table via `waitpid()`, and acts as the sole component authorized to generate and store TLS private keys[span_0](start_span)[span_0](end_span).
  * Keys reside in anonymous memory regions allocated via `mmap(2)` with `MAP_ANON | MAP_PRIVATE` flags, protected by `madvise(MADV_DONTDUMP)` (to prevent inclusion in core dumps), with access strictly restricted via `mprotect(2)` (`PROT_READ` exclusively during key rotation/generation, otherwise set to `PROT_NONE`)[span_1](start_span)[span_1](end_span).
  * Never handles network sockets, protocol parsing, or direct TLS handshakes[span_2](start_span)[span_2](end_span).

* **Level 1 â€” Per-CPU Secondary Masters (Unprivileged Supervisors):**
  * The Master Root spawns a Secondary Master for each available CPU core, bound using process affinity (**CPU Core-Pinning**)[span_3](start_span)[span_3](end_span).
  * Each Secondary Master permanently drops privileges via `setresuid(_sec_master)` immediately upon initialization[span_4](start_span)[span_4](end_span).
  * Operates as an unprivileged local supervisor to balance workloads, handling exclusively the creation (`fork`), termination, and supervision via `waitpid()` of child Level 2 processes[span_5](start_span)[span_5](end_span).

* **Level 2 â€” Per-Task/Session Workers (1 PID per Instance & Extreme Sandboxing):**
  * Each Secondary Master spawns a dedicated child process for every individual user session or task (1 PID per instance)[span_6](start_span)[span_6](end_span).
  * The process inherits unprivileged status from Level 1 and immediately enforces native OpenBSD sandboxing[span_7](start_span)[span_7](end_span):
    * **`unveil()`:** Complete filesystem isolation via an immediate `unveil(NULL, NULL)` lockdown, blocking all read/write disk access[span_8](start_span)[span_8](end_span).
    * **`pledge()`:** Drastic reduction of allowed system calls via `pledge("stdio inet", NULL)`. Any attempt to execute unauthorized syscalls triggers an immediate process termination (`SIGABRT`/`SIGKILL`) by the OpenBSD kernel[span_9](start_span)[span_9](end_span).

---

### 4.2. In-Kernel eBPF Mapping and Decoupled Remediation (Zero-Trust Worker)

In the event of an anomaly or compromise within a Task Worker (Level 2), the system enforces a passive containment model based on kernel status signals and asynchronous reconciliation[span_10](start_span)[span_10](end_span):

1. **Pre-TLS & Initial Registration:**
   * The Task Worker (Level 2) transmits the initial tuple `âŸ¨Source_IP, Port, Worker_PIDâŸ©` to the **Auth Verifier (AV)**[span_11](start_span)[span_11](end_span).
2. **Authentication & Session Update (`AUTH_OK`):**
   * Upon successful authentication, the AV sends the `AUTH_OK` signal to the XDP/eBPF module, including the user's `PseudoUsername_Random`[span_12](start_span)[span_12](end_span).
   * XDP updates and stores this mapping directly inside the kernel-space eBPF `map_session`[span_13](start_span)[span_13](end_span).
3. **Anomaly Handling via BPF Query and Cleanup:**
   * In case of a crash or exploit in a Task Worker (Level 2), the Secondary Master (Level 1) intercepts the critical PID via `waitpid()` and forwards it directly to a service daemon (modeled after the Auth Verifier Daemon)[span_14](start_span)[span_14](end_span).
   * The service daemon queries the Linux eBPF session map using the PID as the lookup key to extract the `PseudoUsername_Random`[span_15](start_span)[span_15](end_span).
   * Upon match, it scans the BPF map and sequentially purges all records associated with that `PseudoUsername_Random`, instantly invalidating any other active sessions belonging to the same user[span_16](start_span)[span_16](end_span).
   * Simultaneously, the daemon updates the identity database (enforcing MFA flags / Rate-Limiting / Ban) and generates audit logs completely decoupled from the fast-path data plane[span_17](start_span)[span_17](end_span).
4. **Precautionary Key Rotation (Level 0):**
   * If the anomaly stems from a memory crash or a critical TLS channel fault, the Master Root (Level 0) regenerates and rotates the TLS key inside its protected memory for all subsequent handshakes, leaving active legitimate sessions on other PIDs undisturbed[span_18](start_span)[span_18](end_span).

---

### 4.3. CPU Core-Pinning, Affinity, and Queue Mapping

* To eliminate context-switching overhead and avoid L1/L2 cache contention, each **Secondary Master (Level 1)** and its child **Task Workers (Level 2)** are bound to a specific CPU core using native kernel affinity primitives[span_19](start_span)[span_19](end_span).
* Core allocation directly mirrors the Receive Side Scaling mapping on the network interface (**NIC RSS Multi-Queue**), ensuring that packets for a given session are consistently processed on the same physical CPU core to preserve cache locality[span_20](start_span)[span_20](end_span).

---

### 4.4. Progressive Implementation (Development Roadmap)

* **Phase 1 â€” Functional Architecture (Linux MVP):** Implementation of a simplified Master1/Master2/Worker model to validate network logic, the eBPF/XDP interface, and the authentication protocol within a coordinated Linux environment[span_21](start_span)[span_21](end_span).
* **Phase 2 â€” Hardening (Production OpenBSD Target):** Deployment of the 3-tiered hierarchy featuring privilege dropping via `setresuid(_sec_master)` at Level 1, 1 PID per task allocation at Level 2, isolated key storage in `mmap`/`mprotect`/`madvise` pages at Level 0, and final sandboxing via `pledge("stdio inet", NULL)` and `unveil(NULL, NULL)`[span_22](start_span)[span_22](end_span).

---

### 4.5. TLS Handshake Security Trade-offs and Secret Management

The architecture delegates the entire TLS 1.3 Handshake to the unprivileged worker process of the **AI Secure Gateway** (OpenBSD) to maximize I/O performance and eliminate latency overhead associated with asynchronous IPC to the Auth Verifier Daemon[span_23](start_span)[span_23](end_span).

#### Comprehensive Security Analysis (Trade-offs)
* **Performance Advantage:** Minimal negotiation latency (0-RTT/1-RTT) handled in-memory on dedicated cores (Core N), significantly reducing I/O loop complexity[span_24](start_span)[span_24](end_span).
* **Past Resilience (Perfect Forward Secrecy):** Thanks to mandatory TLS 1.3 (ECDHE), an event of RAM compromise inside a worker **does not compromise the confidentiality of past sessions**, whose symmetric keys are continuously scrubbed and overwritten via `zeroize`[span_25](start_span)[span_25](end_span).
* **Accepted Risk (Future Active Compromise):** Extraction of the Server private key from worker RAM would allow an attacker to attempt Man-in-the-Middle (MitM) attacks on **future** connections until certificate revocation or rotation occurs[span_26](start_span)[span_26](end_span).

#### Active Mitigation Measures
To reduce the window of potential abuse to the absolute theoretical minimum[span_27](start_span)[span_27](end_span):
1. **Short-Lived Certificates:** Deployment of server certificates with aggressive automated rotation (e.g., 24â€“48 hours validity)[span_28](start_span)[span_28](end_span).
2. **Mutual TLS (mTLS):** Bidirectional cryptographic authentication; possession of the Server private key alone does not allow an attacker to impersonate an authorized client[span_29](start_span)[span_29](end_span).
3. **Restrictive Sandboxing (`pledge` / `unveil` + `zeroize`):** Total inhibition of process execution (`execve`) and filesystem dumps, combined with active `ZeroizeOnDrop` memory scrubbing for ephemeral session keys[span_30](start_span)[span_30](end_span).

---

## 5. Synchronous Fast-Path Promotion (`map_session`) via `AUTH_CK` / `AUTH_UN` / `AUTH_OK` Cycle

Promoting a session to the high-performance state is contingent upon cryptographic/application-layer validation and the completion of a bidirectional handshake cycle[span_31](start_span)[span_31](end_span). All network traffic that does **not** belong to the TCP 3-Way Handshake or the initial authentication phase must find an active match in the **`map_session_v4 / v6`** map; otherwise, the packet is immediately dropped (`XDP_DROP`)[span_32](start_span)[span_32](end_span).

The atomic promotion cycle progresses through three sequential phases[span_33](start_span)[span_33](end_span):

1. **Existence Probe (`AUTH_CK`):** 
   Upon receiving an authentication request, the Auth Verifier Daemon holding the `(Client_IP, Client_Port)` tuple sends an `AUTH_CK` UDP packet to XDP to query the L4 connection state[span_34](start_span)[span_34](end_span).

2. **Hardware Confirmation and Pre-Auth Validation (`AUTH_UN` via `XDP_TX`):** 
   * XDP intercepts `AUTH_CK` on the LAN side and verifies the tuple's presence in **`map_unauth`**[span_35](start_span)[span_35](end_span).
   * If present, it instantly responds to the Auth Verifier via `XDP_TX` with an `AUTH_UN` UDP packet[span_36](start_span)[span_36](end_span).
   * **Negative Outcome:** If the tuple does not exist in `map_unauth` (e.g., connection absent or already expired), XDP does not emit `AUTH_UN`[span_37](start_span)[span_37](end_span). The Auth Verifier marks the credentials as invalid regardless, halts processing without wasting CPU cycles, and logs the anomaly[span_38](start_span)[span_38](end_span).

3. **Final Fast-Path Promotion (`AUTH_OK`):** 
   Only following an `AUTH_UN` confirmation and subsequent application/TLS credential validation does the Auth Verifier Daemon transmit an `AUTH_OK` UDP packet toward the **BoxA:LAN** interface[span_39](start_span)[span_39](end_span):
   * **Silent Interception:** XDP intercepts the `AUTH_OK` packet and silently consumes it (`XDP_DROP`)[span_40](start_span)[span_40](end_span).
   * **Atomic Promotion:** It reads the current state accumulated inside **`map_unauth`** (including the real-time updated `seq_expected` and `window_size`) and copies it directly into the BPF Map **`map_session_v4 / v6`** (Fast-Path)[span_41](start_span)[span_41](end_span).
   * **Removal from Provisional State:** It deletes the `(Client_IP, Client_Port)` tuple from **`map_unauth`**[span_42](start_span)[span_42](end_span).
   * **SYN Protection / DoS Mitigation:** It inserts the same tuple into **`map_blacklist_v4 / v6`**, setting `until_when_ns = 0`[span_43](start_span)[span_43](end_span). This special marker forces the XDP Fast-Path to immediately drop any additional or duplicate `SYN` packets directed toward the active session, protecting the backend from reconnection attempts or SYN-Floods while the session is open[span_44](start_span)[span_44](end_span). The tuple will be atomically removed from the blacklist only upon session closure (when `until_when_ns == 0`)[span_45](start_span)[span_45](end_span).
   * **Counter Update:** It atomically decrements (`__sync_fetch_and_sub`) the `pending_handshake_count` metric on **`map_pending_handshake`**[span_46](start_span)[span_46](end_span).
   * **Line-Rate Forwarding:** From this moment forward, session traffic travels at line-rate on the Fast-Path WAN $\rightarrow$ LAN pipeline, protected by TCP window consistency checks[span_47](start_span)[span_47](end_span).

### 5.1. Fast-Path Specifications, UDP-Driven Teardown, and Race Condition Mitigation

* **Rate-Limiting Exemption for Authenticated Clients:** Clients present in `map_session` travel at full line-rate without undergoing frequency checks or token counting, ensuring maximum throughput[span_48](start_span)[span_48](end_span).
* **In-Window Validation on XDP (RFC 793):** Every incoming data/ACK packet must comply with the TCP window acceptability rule[span_49](start_span)[span_49](end_span):
  $$SEG.SEQ \ge Expected \quad AND \quad SEG.SEQ < Expected + Window\_Size$$
  Packets falling outside this range are dropped at ingress (`XDP_DROP`), protecting OpenBSD from out-of-window ACK Flood attacks[span_50](start_span)[span_50](end_span).
* **Resilience to Blind Sequence Attacks:** If an attacker sends packets with randomized sequence numbers spoofing an active client, XDP drops them on the first CPU cycle[span_51](start_span)[span_51](end_span). The client's entry in `map_session` **remains unaltered and unpurged**, safeguarding legitimate connections from forced disconnections[span_52](start_span)[span_52](end_span).
* **Resilience to Slowloris Attacks and Sandbox Isolation:** 
  * **TLS Failure Tarpit (AI Secure Gateway):** If the TLS/mTLS negotiation fails prior to authentication, the AI Secure Gateway closes the socket by issuing a `TCP RST`[span_53](start_span)[span_53](end_span). XDP on **BoxA:LAN** intercepts the `TCP RST`, purges the session from `map_unauth`, decrements `pending_handshake_count`, and executes a **silent `XDP_DROP` of the `TCP RST`** toward the WAN[span_54](start_span)[span_54](end_span). The attacker remains stalled waiting for a timeout (Tarpit), while OpenBSD and Box A have already cleared their local state[span_55](start_span)[span_55](end_span).
  * **Auth Failure & Sandbox Defense (Auth Verifier Daemon):** If the TLS handshake succeeds but L7 authentication fails, teardown is delegated to the **Auth Verifier Daemon**[span_56](start_span)[span_56](end_span). It sends a `TEARDOWN` UDP packet over the LAN network to **BoxA:LAN** to promote (if necessary) the tuple into `map_blacklist` and clear the BPF state[span_57](start_span)[span_57](end_span). The UDP packet is strictly for internal control plane use (never forwarded to the WAN by XDP), rendering any tampering attempt by a compromised worker futile[span_58](start_span)[span_58](end_span).
* **Timestamp Optimization (1Hz Throttling):** To eliminate unnecessary memory I/O on high-throughput flows, XDP updates the `last_seen_ns` field of a session only if at least 1 second has elapsed since the previous update[span_59](start_span)[span_59](end_span).
* **LAN-Side Window Updates:** Response packets sent from OpenBSD back to the client update the TCP window size (`window_size`) inside the second cache line of the session structure without invalidating the cache line used by the WAN Fast-Path[span_60](start_span)[span_60](end_span).
* **Selective Handling and Bidirectional Synchronization of Closure (`RST`/`FIN`):**
  * **Unauthenticated Sessions (`map_unauth` present):** XDP intercepts `RST` packets generated by OpenBSD and drops them (`XDP_DROP`)[span_61](start_span)[span_61](end_span). The termination of an unvalidated session is managed via a *silent-drop* model, ensuring complete Gateway stealth and preventing information leakage during perimeter scans[span_62](start_span)[span_62](end_span).
  * **Authenticated Sessions (`map_unauth` absent):**
    * **Immediate Termination via `RST`:** Receiving an `RST` packet (from LAN or WAN) triggers immediate forwarding (`XDP_REDIRECT`) to inform the remote peer, alongside an instantaneous atomic removal of the entry from the BPF map (`bpf_map_delete_elem`), freeing resources immediately[span_63](start_span)[span_63](end_span).
    * **Phased `FIN` Termination (`SESSION_CLOSING`):** Upon the passage of the first `FIN` packet (from either direction, LAN or WAN), the session state on that specific Cache Line transitions from `SESSION_ACTIVE` to `SESSION_CLOSING`[span_64](start_span)[span_64](end_span).
      * **Instant Purge on ACK (RFC 793/9293 Compliance):** The simultaneous presence of `SESSION_CLOSING` on both Cache Lines attests to the completion of the bidirectional 4-way teardown handshake in full compliance with TCP standards[span_65](start_span)[span_65](end_span). This authorizes either XDP pipeline/process (LAN or WAN) to instantly purge the session from the BPF map (`bpf_map_delete_elem`) upon receiving the subsequent `ACK` packet, while atomically removing the tuple from `map_blacklist_v4 / v6` if present with `until_when_ns == 0`[span_66](start_span)[span_66](end_span).
      * **Garbage Collector Fallback:** Should teardown fail to complete with the final ACK (e.g., due to packet loss), even a single-sided `SESSION_CLOSING` state enables the Garbage Collector to reclaim the session and its blacklist entry upon the expiration of a reduced *Grace-Timeout* (2â€“5s)[span_67](start_span)[span_67](end_span).

### 5.2. Sanctions and Immediate Termination (`TEARDOWN`)
Upon receiving a `TEARDOWN` UDP packet (credential failure, L7 anomalies, or timeouts)[span_68](start_span)[span_68](end_span):
1. XDP extracts the target tuple `(Client_IP, Client_Port)` from the UDP payload[span_69](start_span)[span_69](end_span).
2. It attempts atomic deletion of the tuple from the limbo map[span_70](start_span)[span_70](end_span):
...
3. **Atomic Race Condition Management (Single Source of Truth Anti Double-Decrement):**
   * **If $\text{ret} == 0$ (Initial deletion succeeded):** The UDP packet intercepted the event first[span_71](start_span)[span_71](end_span). It atomically decrements `pending_handshake_count` on **`map_pending_handshake`** and inserts the tuple/IP into **`map_blacklist`** (or **`map_ip_trespass`** if the host is a repeat offender)[span_72](start_span)[span_72](end_span).
   * **If $\text{ret} == -\text{ENOENT}$ (Entry already deleted by `TCP RST`):** This indicates that a `TCP RST` packet already cleaned up the state microseconds prior[span_73](start_span)[span_73](end_span). The UDP branch **immediately aborts execution without decrementing the counter**, mathematically eliminating any risk of a *double-decrement*[span_74](start_span)[span_74](end_span).
4. XDP executes a **silent `XDP_DROP` on the control UDP packet**[span_75](start_span)[span_75](end_span).
...
| :--- | :--- | :--- | :--- |
| **`map_handshake`** | `BPF_MAP_TYPE_LRU_HASH` | Temporary TCP completion tracking (SYN-ACK $\rightarrow$ ACK). | Fast expiration (3â€“5s) or cleanup via garbage collector. |
...
| **`map_session`** | `BPF_MAP_TYPE_HASH` | **Fast-Path Dual Cache Line**: Authenticated flows exempt from rate-limiting. | Removal via UDP L7 Teardown or passive Grace Period (5â€“10s). |
...
4. XDP drops the command UDP packet (`XDP_DROP`)[span_89](start_span)[span_89](end_span). The duration $T_{infraction}$ is dynamically modulated by the AI Secure Gateway based on the severity of the L7 anomaly[span_90](start_span)[span_90](end_span).

### 7.3. Recidivism Logic and Cumulative Population in User-Space (`map_ip_trespass`)
The `map_ip_trespass` map is populated and processed exclusively by the **User-Space Garbage Collector on Linux**[span_91](start_span)[span_91](end_span):
1. **Tracking and Event-Sourcing:** The daemon periodically inspects the **active bans present in `map_blacklist`**[span_92](start_span)[span_92](end_span). Because L7 session entries are purged immediately upon termination, the daemon focuses strictly on the frequency and concurrency of L3/L4 infractions originating from the same source IP[span_93](start_span)[span_93](end_span).
2. **Cumulative Suspension Calculation:** If multiple or simultaneous infractions occur from the same IP within the observation window (e.g., multi-port attack attempts or aggressive reconnections), the daemon calculates a progressive time penalty[span_94](start_span)[span_94](end_span):
   $$T_{trespass} = \sum_{i=1}^{k} T_{infraction\_i} \times Factor_{recidiva}$$
3. **Promotion to Trespass:** Upon exceeding the recidivism threshold, the daemon inserts the source host IP (`/32` or `/128`) into `map_ip_trespass` with an extended expiration timestamp (on the order of hours/days), blocking any handshake attempt before traffic can reach the blacklist check or the OpenBSD stack[span_95](start_span)[span_95](end_span). In-depth forensic analysis and identity correlation remain strictly confined to the asynchronous audit logging pipeline[span_96](start_span)[span_96](end_span).
...
* **Targeted DoS and Hijacking Resistance:** Exempting successfully authenticated flows from aggressive rate-limiting checksâ€”combined with strict TCP window validation (RFC 793)â€”guarantees maximum throughput for legitimate clients while ensuring complete immunity to de-authentication attempts via spoofed garbage packets.
